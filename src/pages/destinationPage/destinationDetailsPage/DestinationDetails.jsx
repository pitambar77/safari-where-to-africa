...
const DestinationDetails = () => {
  ...
  //     try {
  //       setLoading(true);
  //       const res = await getRegionBySlug(areaName);
  //       setRegion(res.data);
  //     } catch (err) {
    ...
    <>
        ...
        {trips?.length > 0 && (
          <ParticularDestinationPackage
            data={trips.map((trip) => ({
              ...
              price: trip.price,
              country: region?.destinationId?.name || "",
            }))}
            CardComponent={SafariCard}
      ...
      {/* {experiences?.length > 0 && (
        <ThingsToDoSection
          ...
          description1="Explore wildlife, culture, and landscapes."
          description2="Discover guided safaris, cultural tours, and adventure trails."
