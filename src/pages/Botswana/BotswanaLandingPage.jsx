...
//   useEffect(() => {
//     axios.get(`/destinations/slug/${slug}`).then((res) => setDestination(res.data));
//   }, [slug]);
...
const experienceData = experiences?.[0];
  ...
  return (
    <div>
      ...
      <div className="bg-[#faf5e9]">
        <Overview
          title={experienceData?.overview?.title || "Welcome to Botswana"}
          subtitle={experienceData?.overview?.subTitle}
          description={experienceData?.overview?.description}
        />
      ...
      {/* {accommodations?.length > 0 && (
        <AccommodationGrid
          ...
          data={accommodations.map((acc) => ({
            ...
            onCardClick={(id) => navigate(`/accommodation/${id}`)} // 👈 Add this
        />
      )} */}
